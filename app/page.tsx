...
function OutlineBtn(props: React.ButtonHTMLAttributes<HTMLButtonElement> & { pressed?: boolean }) {
...
/** Accessible, dependency-free tooltip that works in Notion iframe */
...
type Task = { id: string; name: string; due: string | null; hasRule: boolean; overdue: boolean; done?: boolean; parentDb?: string; created?: string };
...
export default function Page() {
  ...
  // inline error below buttons (non-blocking)
  ...
  /** ---------------------------
   *  OAuth handoff adoption (iframe-safe)
   *  --------------------------*/
  useEffect(() => {
    // URL handoff fallback
    ...
    async function onMsg(e: MessageEvent) {
      if (e?.data?.type === "recurio:oauth-complete") {
        ...
        if (hh) {
          try {
            await fetch(`/api/session/adopt?h=${encodeURIComponent(hh)}`, { method: "POST", credentials: "include", cache: "no-store" });
          } catch {}
        }
  ...
  function openConnectPopup() {
    window.open("/api/oauth/start", "recurio-oauth", "width=600,height=750,noopener,noreferrer");
  }
  ...
  async function onResetManaged(mode?: "archive") {
    ...
    setStatus(mode === "archive" ? "Archiving all rules…" : "Ensuring Recurio (Managed) + Rules DB…");
    try {
  ...
  async function onSyncNow() {
    ...
    try {
      ...
      await openDb(dbId); // refresh list to include newly-created rule tasks
    } catch (e: any) {
  ...
  // initial connection ping (don’t block UI)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        ...
        {/* inline error rows (under their respective controls) */}
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16 }}>
          ...
          <ul style={{ listStyle: "none", margin: "8px 0 0", padding: 0 }}>
            {dbs.map((d) => (
              <li key={d.id} style={{ marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
                <OutlineBtn pressed={dbId === d.id} onClick={() => openDb(d.id)} style={{ padding: "6px 10px" }}>
                  Open
                </OutlineBtn>
            ...
            {!dbs.length && (
              <li style={{ color: "#666", fontSize: 13 }}>
                No databases yet. Share your Tasks DB with <b>Recurio</b> (Can edit), then “Refresh databases”.
              </li>
            )}
          </ul>
        </div>
        ...
        <div style={{ border: "1px solid #111", padding: 12, borderRadius: 6 }}>
            ...
            {tasks.map((t) => (
              ...
              >
                <label style={{ flex: 1, cursor: "pointer" }}>
                  <input type="radio" name="pick" checked={taskId === t.id} onChange={() => setTaskId(t.id)} /> <b>{t.name}</b>{" "}
                  &nbsp; <small>Due: {fmt(t.due)}</small>
                </label>
                {t.overdue && (
                  <span
                    title="Due date has passed (last 14 days)"
                    style={{ fontSize: 12, border: "1px solid #111", padding: "2px 6px", background: "#ffefef" }}
                  >
                    Overdue
                  </span>
                )}
          ...
          {/* inline error row for make recurring */}
